...
def boxplot_year_hs(df,df_hs):
    df=df[["AC_YEAR",(df.columns[3])]]
    df = df.set_index([df.groupby('AC_YEAR').cumcount(), 'AC_YEAR'])[(df.columns[1])].unstack()
    df=df.round(decimals=3)
    df=df.dropna(axis=0, how='any')
    df.rename(index=str, columns={2010:"Y2010_g",2011:"Y2011_g",2012:"Y2012_g",2013:"Y2013_g",2014:"Y2014_g",2015:"Y2015_g"},inplace=True)
    
    df_hs=df_hs[["AC_YEAR",(df_hs.columns[3])]]
    df_hs = df_hs.set_index([df_hs.groupby('AC_YEAR').cumcount(), 'AC_YEAR'])[(df_hs.columns[1])].unstack()
    df_hs=df_hs.round(decimals=3)
    df_hs=df_hs.dropna(axis=0, how='any')
    df_hs.rename(index=str, columns={2010:"Y2010",2011:"Y2011",2012:"Y2012",2013:"Y2013",2014:"Y2014",2015:"Y2015"},inplace=True)
...
def passed_first_all_hs(df,df_hs):
    df=df[["AC_YEAR","PASSED"]]
    df=df.groupby(["AC_YEAR"])
    df=df.mean()
    df=df.reset_index()
    df=df.round(decimals=2)
    df.rename(index=str, columns={"PASSED":"Passed global"},inplace=True)

    df_hs=df_hs[["AC_YEAR","PASSED"]]
    df_hs=df_hs.groupby(["AC_YEAR"])
    df_hs=df_hs.mean()
    df_hs=df_hs.reset_index()
    df_hs=df_hs.round(decimals=2)
    df_hs.rename(index=str, columns={"PASSED":"Passed"},inplace=True)

    df_total=pd.merge(df,df_hs,how='inner')
    df_total.to_csv("static/CSV/Chart_data/passed_first_all.csv", index=False)
